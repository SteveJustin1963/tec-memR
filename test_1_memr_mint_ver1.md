...
2. **Enter Code:** Upload via serial terminal (e.g., minicom at 4800 bps). Strip comments first:  
   ```
   0p!0pol_set!1pol_reset!:Sw!pol_set@p/O1p/Ow()0p/O`SET pulse applied`/N;:Rw!pol_reset@p/O1p/Ow()0p/O`RESET pulse applied`/N;:D10w!pol_set@p/O1p/Ow()0p/O`READ pulse applied`/N;:Ipol!count!w!count(pol@p/O1p/Ow()0p/O100())`Incremental pulses done`/N;
   ```  
...
- `0 pol_set !` : Polarity for SET (0 = positive/low resistance).  
- `1 pol_reset !` : Polarity for RESET (1 = negative/high resistance).  
...
> 1 pol_set !  // Invert polarity if hardware requires (SET=1)
